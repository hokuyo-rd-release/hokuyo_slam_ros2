...
import csv
import numpy as np  # NumPy を使用して配列を処理します
...
if __name__ == "__main__":
        ...
        csv_file_path = '/home/takahashi/github/hokuyo_slam/data/testlog.csv'

        # path to the bagfile
        bag_file = '/home/takahashi/github/hokuyo_slam/rosbag/nakanoshima2024_second_sensor_no_ylm/nakanoshima2024_second_sensor_no_ylm_0.db3'
        ...
        t, msgs = getAllMessagesInTopic(c, topic_name, print_out=False)
        ...
        with open(csv_file_path, 'w', newline='') as csvfile:
            ...
            # Write the header row for position_covariance
            # header = ['timestamp'] + [f'covariance_{i}' for i in range(9)]
            header = [f'covariance_{i}' for i in range(9)]
            csv_writer.writerow(header)

            for timestamp, message in zip(t, msgs):
                    deserialized_msg = deserialize_message(message, msg_type)

                    # Extract the position_covariance array
                    covariance_array = deserialized_msg.position_covariance

                    # Write the timestamp and the covariance values to the CSV file
                    row_data = list(covariance_array)
                    row_data = list(covariance_array)
                    csv_writer.writerow(row_data)

                    # Print the timestamp and the covariance array
                    # print(f"Timestamp: {timestamp}")
                    print(f"Position Covariance: {covariance_array}")
